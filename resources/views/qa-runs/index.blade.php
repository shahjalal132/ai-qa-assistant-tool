<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <form id="bulk-actions-form" action="{{ route('qa-runs.bulk-action') }}" method="POST">
                @csrf
                ...
                <div class="bg-white/80 backdrop-blur-md border border-gray-100 shadow-xl sm:rounded-2xl overflow-hidden relative">
                    ...
                    <div id="bulk-action-bar" class="hidden absolute top-0 left-0 right-0 z-10 bg-[#16a085]/95 backdrop-blur-xl border-b border-white/20 px-8 py-3 flex items-center justify-between animate-in slide-in-from-top duration-300">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex items-center gap-2">
                                <button type="button" onclick="submitBulk('activate')" class="px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-white text-[10px] font-bold uppercase tracking-wider transition-all">
                                    {{ __('Activate') }}
                                </button>
                                <button type="button" onclick="submitBulk('deactivate')" class="px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-white text-[10px] font-bold uppercase tracking-wider transition-all">
                                    {{ __('Deactivate') }}
                                </button>
                                <button type="button" onclick="submitBulk('retry')" class="px-3 py-1.5 rounded-lg bg-amber-400/20 hover:bg-amber-400/30 text-amber-200 text-[10px] font-bold uppercase tracking-wider transition-all">
                                    {{ __('Retry') }}
                                </button>
                                <button type="button" onclick="submitBulk('delete')" class="px-3 py-1.5 rounded-lg bg-red-400/20 hover:bg-red-400/30 text-red-200 text-[10px] font-bold uppercase tracking-wider transition-all">
                                    {{ __('Delete') }}
                                </button>
                            </div>
                        </div>
                            ...
                            <tbody class="divide-y divide-gray-50 text-sm">
                                @forelse ($runs as $run)
                                    <tr class="hover:bg-gray-50/80 transition-colors group">
                                        <td class="px-6 py-4">
                                            <input type="checkbox" name="ids[]" value="{{ $run->id }}" class="run-checkbox rounded border-gray-300 text-[#16a085] focus:ring-[#16a085]/20 h-4 w-4 transition-all cursor-pointer">
                                        </td>
            ...
            <script>
                ...
                function submitBulk(action) {
                    if (action === 'delete') {
                        if (!confirm('{{ __("Are you sure you want to delete the selected runs?") }}')) return;
                    }
