            ...
            <div class="bg-white/80 backdrop-blur-md border border-gray-100 shadow-xl sm:rounded-3xl overflow-hidden">
                <div class="p-8 sm:p-10 space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div>
                            <h3 class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">{{ __('Gemini model') }}</h3>
                            <div class="flex items-center gap-3">
                                <div class="h-10 w-10 rounded-xl bg-[#1abc9c]/10 flex items-center justify-center text-[#16a085]">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <div>
                                    @if ($run->aiModel)
                                        <p class="text-sm font-bold text-gray-900 font-mono">{{ $run->aiModel->name }}</p>
                                        <a href="{{ route('models.show', $run->aiModel) }}" class="text-[10px] font-bold text-[#16a085] hover:text-[#1abc9c] uppercase transition-colors">{{ __('View model') }}</a>
                                    @else
                                        <p class="text-sm font-bold text-gray-400 font-mono">{{ __('Not set (legacy run)') }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
