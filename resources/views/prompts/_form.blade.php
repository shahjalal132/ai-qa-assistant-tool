<div class="space-y-6">
    ...
    <div>
        ...
        <div class="relative group">
            <textarea id="system_instruction" name="system_instruction" rows="12" required
                class="mt-1 block w-full border-gray-200 focus:border-[#1abc9c] focus:ring-[#1abc9c]/20 rounded-xl shadow-sm text-sm font-mono bg-gray-50/30 transition-all focus:bg-white leading-relaxed"
                placeholder="{{ __('Enter the core instructions for the AI...') }}">{{ old('system_instruction', $prompt->system_instruction ?? '') }}</textarea>
            <div class="absolute bottom-3 right-3 text-[10px] font-bold text-gray-300 pointer-events-none uppercase tracking-widest">{{ __('Instruction set') }}</div>
        </div>
    ...
    <div>
        ...
        <div class="relative group">
            ...
            <div class="absolute bottom-3 right-3 text-[10px] font-bold text-gray-300 pointer-events-none uppercase tracking-widest">{{ __('JSON Schema') }}</div>
        </div>
